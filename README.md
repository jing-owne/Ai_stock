...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| **v2.8.0** | 2026-07-15 | 4聚合策略重构落地；新增 legacy_mode 新旧并行切换；统一聚合权重与运行时配置；版本号全局统一 |
| **v2.6.8** | 2026-06-09 | 分支清理(删除bond-reminder)；回测脚本独立入库(backtest/)含标的去敏；打新日历akshare列名兼容修复；volume_bonus NameError修复；合并marcus→main |
| **v2.6.7** | 2026-06-09 | 高位防套转向成交量信号：新增连续放量加分、防套惩罚全面放宽(40→20)、位置评分放缓 |
| **v2.6.6** | 2026-06-08 | 涨幅过滤放宽(-5%~7%)；高位防套放宽；K线预过滤删除change_pct>0限制 |
