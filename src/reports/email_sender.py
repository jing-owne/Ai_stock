    ...
    <div style="max-width: 390px; margin: 0 auto; background-color: white; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
        ...
        <div style="background: linear-gradient(135deg, #F8FAFC 0%, #F1F5F9 100%); padding: 16px 20px; border-top: 2px solid #E2E8F0;">
            <div style="text-align: center; margin-bottom: 12px;">
                <span style="display: inline-block; padding: 6px 16px; background: linear-gradient(135deg, #4F46E5 0%, #6366F1 100%); color: white; border-radius: 20px; font-size: 12px; font-weight: 600;">Marcus量化选股小助手</span>
            </div>
...
<body style="background-color: #F1F5F9; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'PingFang SC', sans-serif;">
    ...
    <div class="email-container" style="max-width: 600px; margin: 0 auto; background-color: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <div style="background: linear-gradient(135deg, #F8FAFC 0%, #F1F5F9 100%); padding: 20px 24px; border-top: 2px solid #E2E8F0;">
            <div style="text-align: center; margin-bottom: 12px;">
                <span style="display: inline-block; padding: 8px 20px; background: linear-gradient(135deg, #4F46E5 0%, #6366F1 100%); color: white; border-radius: 20px; font-size: 13px; font-weight: 600;">Marcus量化选股小助手</span>
            </div>
            <p style="margin: 0 0 6px 0; text-align: center; color: #94A3B8; font-size: 12px;">⚠️ 本报告由AI自动生成，仅供参考</p>
...
class EmailSender:
    ...
    def generate_email_content(self, stock_results: List[Dict], strategy_config: Dict = None) -> str:
        ...
        # 4. 股票选择
        content_lines.append("【股票选择】")
        content_lines.append("")
        content_lines.append("策略命中的TOP15：")
        content_lines.append("")
        
        if stock_results:
            # 表头
            content_lines.append("| 代码 | 名称 | 评分 | 涨跌幅 |")
            content_lines.append("|------|------|------|--------|")
            ...
            for i, stock in enumerate(stock_results[:15], 1):
                ...
                content_lines.append(f"| {symbol} | {name} | {score:.1f} | {change_pct:+.2f}% |")
        ...
        if stock_results:
            for i, stock in enumerate(stock_results[:5], 1):
                ...
                content_lines.append(f"{i}. {symbol} {name}")
                content_lines.append(f"   现价: {current_price:.2f} ({change_pct:+.2f}%)")
                content_lines.append(f"   建议买入价: {suggest_buy_price:.2f} | 止损: {stop_loss:.2f} | 止盈: {take_profit:.2f}")
                content_lines.append("")
        ...
        if stock_results:
            ...
            content_lines.append(f"▶ 今日共筛选 {len(stock_results[:15])} 只股票")
    ...
    def send_report(
        ...
        subject = f"[Marcus量化选股小助手] {date_str} 动态策略报告"
